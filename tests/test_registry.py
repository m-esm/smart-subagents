    ...
    def test_record_writes_a_ledger_line_naming_the_fourth_worker(self):
        with temp_env() as te:
            ...
            (task_dir / "outcome.json").write_text(
                json.dumps({"schema_version": 1, "verify": {"verdict": "pass"}})
                + "\n"
            )
            env = registry_env(te)
...
class RegistryValidationTests(unittest.TestCase):
    ...
    def test_env_keep_item_must_be_an_uppercase_identifier(self):
        with temp_env() as te:
            rc, out, err = self._validate(
                te,
                lambda doc: doc["workers"]["fakecli"]["run"].__setitem__(
                    "env_keep", ["HOME", "not-an-env"]
                ),
            )
            self.assertEqual(rc, 1, out)
            self.assertIn("run.env_keep", err)

    def test_env_keep_must_be_an_array(self):
        with temp_env() as te:
            rc, out, err = self._validate(
                te,
                lambda doc: doc["workers"]["fakecli"]["run"].__setitem__(
